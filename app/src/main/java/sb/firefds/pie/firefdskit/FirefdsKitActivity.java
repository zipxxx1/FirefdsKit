...
import sb.firefds.pie.firefdskit.app.BuildConfig;
...
public class FirefdsKitActivity extends AppCompatActivity
    ...
    public void onRestoreDefaults() {
        ...
        if (!Utils.isOmcEncryptedFlag()) {
            XCscFeaturesManager.applyCscFeatures(sharedPreferences, appContext);
        }

        recreate();
        Toast.makeText(activity, R.string.defaults_restored, Toast.LENGTH_LONG).show();
        RebootNotification.notify(appContext, 999, false);
    }
    ...
    private void showSaveDialog() {
        ...
        saveDialog.showDialog(activity, appContext, findViewById(android.R.id.content));
    }
    ...
    private static class CheckRootTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                suAvailable = Utils.isRootAvailable();
